...
## 12. 6 bug thật tìm được trong `prune.py`/`tasks_pruned.py`/`block_pruned.py`/`head_pruned.py`
...
**Việc cần làm tiếp** (chưa làm): benchmark toy ở mục 11 (so sánh distill vs
finetune) cần chạy lại từ đầu trên `prune.py` đã sửa — mọi kết luận ở mục 11
dựa trên checkpoint pruned bị lỗi (bug 1-6), không còn giá trị tham chiếu.
